...
class Processor:
    ...
    def read_cache(self, index, tag):
        ...
        if state != 'i':
            ...
            clock = directory.request(index, tag, self.p, clock)
    ...
    def write_cache(self, index, tag):
        ...
        if state == 'm':
            ...
            clock += directory.send_invalidates(index, tag, self.p, state)
            clock += self.update_state(index, tag, 'm')
            return clock + 1

    # Called by other processors via the directory, this is to share the data in this cache
    def send(self, index, tag, receiver):

        clock = self.read_cache(index, tag)
        if self.lines[index]['state'] == 'm':
            statistics['Coherence-writebacks'] += 1
        self.update_state(index, tag, 's')

        return clock + ((receiver - self.p) % 4) * 3  # This calculates the total latency between the receiving and
        # sending processor

    # Called by other processors via the directory, this is to invalidate the data in this cache
    def invalidate(self, index, tag, receiver, get_data):

        statistics['Invalidations-sent'] += 1
        clock = self.update_state(index, tag, 'i')
        return clock + ((receiver - self.p) % 4) * 3 + int(get_data)  # This calculates the total latency between the
        # receiving and sending processor and also adds an extra cycle if there needs to be a cache read
...
class Directory:
    ...
    def request(self, index, tag, p, clock=0):
        ...
        i = (p - 1) % 4
        while i != p:  # While loop, loops through all other processors looking for sharers
            if self.lines[index][tag][i] != 'i':
                par_clocks.append(clock + processor[i].send(index, tag, p) + 5)  # Add 5 for latency to access other
                # processor

            i = (i - 1) % 4  # Go to prev processor

        if par_clocks:  # If a sharer was found as this list is not empty
            at.type_accessed('remote')
            return par_clocks[0]  # We only use the latency for the nearest processor
    ...
    def send_invalidates(self, index, tag, p, prev_state, clock=0):
        ...
        if tag not in self.lines[index]:
            self.lines[index][tag] = ['i', 'i', 'i', 'i']
        i = (p - 1) % 4
        get_data = prev_state == 'i'  # Get data if the prev state is invalid
        while i != p:  # While loop, loops through all other processors looking for sharers, starting with nearest
            if self.lines[index][tag][i] != 'i':
                par_clocks.append(processor[i].invalidate(index, tag, p, get_data) + 5)
                get_data = False  # We have gotten the data now, no need to check slower processors
            i = (i - 1) % 4

        if get_data:  # We still need the data, so have to access main memory
            par_clocks.append(20)
            at.type_accessed('off-chip')
        else:
            at.type_accessed('remote')

        return clock + max(par_clocks)  # We add the latency of the slowest processor or operation happening in parallel
